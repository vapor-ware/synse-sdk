...
// Device is the unified DeviceConfig set for the plugin. It is globally accessible to the SDK.
...
func ProcessDeviceConfigs(dynamicRegistrar func(map[string]interface{}) ([]*DeviceConfig, error)) error { // nolint: gocyclo
	...
	var dynamicCtxs []*Context

	// Get device configs from dynamic registration
	dynamicCfgs, err := dynamicRegistrar(Plugin.DynamicRegistration.Config)
	...
	Device = unifiedCtx.Config.(*DeviceConfig)
	return nil
}
